...
class Component(ComponentBase):
    ...
    def run(self):
        ...
        logging.info(f"Request Range: {start_date_str} to {end_date_str}")
        accounts = params.get(KEY_ACCOUNTS, {})
        if not accounts:
            raise UserException(
                "The authorized account has to have a linked My Google Business account with "
    ...
    def get_oauth_token(config):
        ...
        if response.status_code != 200:
            raise UserException(
                f"Unable to refresh access token. Please reset the account authorization: {response.text}"
            )
    ...
    def list_accounts(self):
        ...
        gmb = GoogleMyBusiness(access_token=oauth_token, data_folder_path=self.data_folder_path)
        try:
            gmb.list_accounts()
        except GoogleMyBusinessException:
            raise UserException(
                "Failed to retrieved Google My Business accounts for which the authorized user has management rights."
            )
        ...
        if gmb.account_list:
            for account in gmb.account_list:
                if account.get("name", None) and account.get("accountName", None):
                    accounts.append({"label": account.get("accountName"), "value": account.get("name")})
            return accounts
        else:
            raise UserException(
                "Authorized account does not have any Google My Business accounts with management rights."
            )
